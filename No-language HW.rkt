...
(define no-parser
  (lambda (no-code)
    (cond
      ...
      ((eq? (car no-code) 'ask)
       (list 'ask-exp
             (arith-bool-parser (caadr no-code))
  ...
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car parsed-no-code) 'ask-exp)
       (run-parsed-ask-code (run-parsed-code(cadr parsed-no-code) env) (run-parsed-code (caddr parsed-no-code) env) (run-parsed-code (cadddr parsed-no-code) env)))             
      (else
